# 奄美大島旅行しおりアプリケーション 要件定義書 Ver3

## 1. プロジェクト概要

### 1.1 目的
奄美大島旅行のしおり機能を提供するWebアプリケーションを開発する。参加メンバー間で旅行工程の進捗を共有し、奄美大島歩き方（Google Mapsを用いた案内機能）を実装する。
...
### 1.3 技術スタック
- **フロントエンド**: Next.js 14.x / React 18.x / TypeScript
...
- **地図機能**: Google Maps JavaScript API（@react-google-maps/api）
- **デプロイ**: Vercel
...
- ダークモード/ライトモード切替機能（next-themes使用）
- 印刷機能
- トップへスクロールボタン
- **背景画像の適用** ✅ **Ver3追加**
  - 全ページに奄美大島の写真（haikei.png）を背景として適用
  - 固定背景（fixed）でスクロール時も背景が固定
  - テキスト可読性のため、グラデーションオーバーレイを適用
    - ライトモード: 透明度75%の薄い青のグラデーション
    - ダークモード: 透明度85%の濃い青のグラデーション
...
#### 2.2.3 奄美大島歩き方（地図：FromTo）（✅ 実装済み）

1. **基本機能** ✅
   - 奄美大島の地図を表示
   - Google Maps JavaScript APIを使用（@react-google-maps/api）
   - 地図操作（ズーム、ドラッグ移動）
...
3. **マーカー表示** ✅
   - 日程に含まれる主要地点にマーカーを表示
   - マーカーの色分け（観光地:青、食事処:赤、ホテル:緑、アクティビティ:黄）
   - マーカーに時刻ラベルを表示
...
- **UI改善（Ver3追加）** ✅
  - **ボタンカラーの最適化** ✅
    - Primaryボタン: 鮮明なコーラルレッド→オレンジのグラデーション（`#FF6B6B` → `#FF8E53`）
    - シャドウを強化（`shadow-lg`、不透明度を上げて強調）
    - フォントを太字（`font-semibold`）に変更してアクティブ感を向上
    - インアクティブに見える問題を解決
...
1. ✅ トップページのレイアウト作成
2. ✅ メニュー実装（ツアー内容、ツアー日程、奄美大島歩き方）
3. ✅ ツアー内容セクションの実装（既存データの移行）
4. ✅ ツアー日程セクションの実装（チェック機能、ローカル保存）
5. ✅ 観光地・食事処のWEBサイトリンク機能の実装
6. ✅ 持ち物セクションの実装

### Phase 2: 奄美大島歩き方（地図）機能 ✅ **完了**

1. ✅ Google Maps JavaScript APIの統合（@react-google-maps/api）
2. ✅ 奄美大島の地図表示
...
### Phase 4: リファインメント ✅ **部分実装**

1. ✅ UI/UXの改善
   - ✅ 背景画像の追加（奄美大島の写真）
   - ✅ ボタンカラーの最適化（鮮明なコーラルレッド→オレンジ）
   - ✅ レスポンシブデザインの改善
2. ⚠️ パフォーマンス最適化（未測定）
...
- [x] モダンフレームワークの採用（Next.js/Reactで実装完了）
...
| 2024-12-XX | 3.0 | **Ver3作成**: Next.js/React実装を反映、「GoogleMap」→「奄美大島歩き方」に変更、背景画像追加、UI改善（ボタンカラー最適化）を記載 | - |
...
### ✅ 実装完了
- Phase 1: 基本機能（100%）
- Phase 2: 奄美大島歩き方（地図）機能（100%）
- Phase 2.5: 割り勘・立替機能（100%）
- Phase 4: リファインメント（50% - UI改善、背景画像、ボタンカラー最適化完了）

### ⚠️ 未実装
- Phase 3: 進捗共有機能（0%）
- Phase 4: リファインメント（残り50% - パフォーマンス測定、エラーハンドリング強化、テスト、APIキー環境変数化）

### 主要な制約事項
1. **データ共有機能が未実装**: 現在は各端末のLocalStorageに個別保存
2. **APIキーの環境変数化未実装**: コード内に直接記述（セキュリティリスク）
3. **認証機能未実装**: アクセス制限なし

### Ver3での主な変更点
1. **UI改善**
   - 全ページに奄美大島の写真を背景として適用
   - ボタンカラーを鮮明なコーラルレッド→オレンジのグラデーションに変更
   - シャドウとフォントウェイトを強化してアクティブ感を向上
2. **名称変更**
   - 「GoogleMap」→「奄美大島歩き方」に変更（ページタイトル、ナビゲーションメニュー、説明文）
3. **技術スタックの明確化**
   - Next.js 14.x / React 18.x / TypeScript / Tailwind CSS を使用
   - @react-google-maps/apiを使用したGoogle Maps統合
...
**注意**: この要件定義書Ver3は、現在の実装状況を反映しています。Phase 3/4の実装進捗に合わせて随時更新が必要です。
